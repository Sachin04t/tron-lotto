...
contract LotteryContract is ReentrancyGuard, Ownable {
    ...
    constructor(address _tokenAddress, uint256 _ticketPrice, uint256 _drawInterval) Ownable(msg.sender) {
    ...
    function drawWinner() external nonReentrant {
        ...
        uint256 winnerIndex = uint256(keccak256(abi.encodePacked(block.prevrandao, block.timestamp, participants.length))) % participants.length;
